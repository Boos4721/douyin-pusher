"""
小云雀 (Xyq) 视频生成后端 - 网页版自动化

支持模型:
- seedance-2.0: Seedance 2.0
- seedance-2.0-fast: Seedance 2.0 Fast

网址: https://xyq.jianying.com
...
class XyqBackend:
    ...
    def __init__(self):
        config = load_config()
        xyq_cfg = config.get("video_backends", {}).get("xyq", {})

        self.cookies_path = os.path.expanduser(xyq_cfg.get(
            "cookies_path",
            "~/.dy/cookies/xyq.json"
        ))
    ...
    async def _generate_async(
        ...
        """异步生成视频 - 基于 lanshu-waytovideo 项目"""
        ...
        print(f"[小云雀] 发起视频生成任务: {prompt[:50]}...")
        ...
        async with async_playwright() as pw:
            ...
            is_logged_in = '小云雀助你' in page_content or '新对话' in page_content
            if not is_logged_in:
                print("  请先在浏览器中登录小云雀")
                await browser.close()
                raise Exception("未登录，请先导出 xyq.jianying.com 的 cookies")
