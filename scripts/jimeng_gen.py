...
class JimengVideoGenerator:
    ...
    def __init__(self, ak: Optional[str] = None, sk: Optional[str] = None):
        ...
        if not self.ak or not self.sk:
            raise ValueError(
                "VOLC_ACCESSKEY or VOLC_SECRETKEY is not set. Please provide them or set environment variables."
            )
    ...
    def generate(
        ...
        try:
            ...
            if "code" in resp and resp["code"] != 10000:
                raise Exception(
                    f"API Error: {resp.get('message', 'Unknown Error')} (Code: {resp['code']})"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ak",
        default=None,
        help="火山引擎 Access Key，若不传则读取 VOLC_ACCESSKEY 环境变量",
    )
    parser.add_argument(
        "--sk",
        default=None,
    ...
    parser.add_argument(
        ...
        default=5,
        choices=[5, 10],
        help="视频时长 (5 或 10，默认 5)",
    )
    parser.add_argument(
        "--aspect_ratio",
        default="16:9",
        choices=["16:9", "4:3", "1:1", "3:4", "9:16", "21:9"],
        help="文生视频的长宽比 (默认 16:9)",
    )
